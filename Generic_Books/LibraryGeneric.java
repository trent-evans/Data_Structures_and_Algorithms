	...
	private ArrayList<LibraryBookGeneric<Type>> library;
	  ...
	  public ArrayList<LibraryBookGeneric<Type>> lookup(Type holder) {
	    // FILL IN -- do not return null
		  ArrayList<LibraryBookGeneric<Type>> booksHeldByHolder = new ArrayList<>();
		  for(int x = 0; x < library.size(); x++) {
			  if(library.get(x).getHolder() != null && library.get(x).getHolder().equals(holder)) {
				  booksHeldByHolder.add(library.get(x));
			  }
		  }
	    return booksHeldByHolder;
	  }
	  ...
	  public boolean checkout(long isbn, Type holder, int month, int day, int year) {
	    // FILL IN -- do not return false unless appropriate
		  for(int x = 0; x < library.size(); x++) {
			  if(library.get(x).getIsbn() == isbn) {
				  LibraryBookGeneric<Type> bookToCheckOut = library.get(x);
				  if(bookToCheckOut.getHolder() == null) {
					  bookToCheckOut.checkBookOut(holder, month, day, year);
					  return true;
				  }
			  }
		  }
	  ...
	  public boolean checkin(long isbn) {
	    // FILL IN -- do not return false unless appropriate
		  for(int x = 0; x < library.size(); x++) {
			  if(library.get(x).getIsbn() == isbn) {
				  LibraryBookGeneric<Type> bookToCheckIn = library.get(x);
				  if(!(bookToCheckIn.getHolder() == null)) {
					  bookToCheckIn.checkBookIn();
					  return true;
				  }
			  }
		  }
	  ...
	  public boolean checkin(Type holder) {
	    // FILL IN -- do not return false unless appropriate
		  boolean iCheckedBooksIn = false;
		  for(int x = 0; x < library.size(); x++) {
			  if(library.get(x).getHolder() != null && library.get(x).getHolder().equals(holder)) {
				  library.get(x).checkBookIn();
				  iCheckedBooksIn = true;
			  }
		  }
	  ...
	  public ArrayList<LibraryBookGeneric<Type>> getOverdueList(int month, int day,
	    ...
	    for(int x = 0; x < library.size(); x++) {
	    	if(library.get(x).getDueDate() != null && library.get(x).getDueDate().compareTo(overdueDate) < 0) {
	    		overdueList.add(library.get(x));
	    	}
	    }
