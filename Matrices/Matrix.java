...
public class Matrix {
  ...
  public boolean equals(Object other) {
    ...
    if(this.numColumns != matrix.numColumns) {
    	return false;
    }else if(this.numRows != matrix.numRows) {
    	return false;
    }
    
    for(int x = 0; x < this.numRows; x++) {
    	for(int y = 0; y < this.numColumns; y++) {
    		if(this.data[x][y] != matrix.data[x][y]) {
    			return false;
    		}
    	}
    }
  ...
  public Matrix times(Matrix matrix) {
     ...
     */
	  ...
	  Matrix resultMatrix = new Matrix(new int[this.numRows][matrix.numColumns]);
	  
	  for(int x = 0; x < this.numRows; x++) {
		  for(int y = 0; y < matrix.numColumns; y++) {
			  int sum = 0;
			  for(int z = 0; z < this.numColumns; z++) {
				  sum += (this.data[x][z] * matrix.data[z][y]);
			  }
			  resultMatrix.data[x][y] = sum;
		  }
	  }
  ...
  public Matrix plus(Matrix matrix) {
     ...
     */
	  ...
	  for(int x = 0; x < this.numRows; x++) {
		  for(int y = 0; y < this.numColumns; y++) {
			  resultMatrix.data[x][y] = this.data[x][y] + matrix.data[x][y];
		  }
	  }
